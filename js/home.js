...
(function ($) {
  jQuery(document).ready(function () {
    ...
    // ================== Skill cards =====================
    var skillCardHolder = document.getElementById("skill-card-holder");
    if (skillCardHolder != null && skillCardHolder.children.length != 0) {
      skillCards = $(".filtr-skills").filterizr({
        layout: 'sameWidth',
        setupControls: false 
      });
      $('#skill-filter-buttons button').click(function () {
        var filter = $(this).data('filter');
        skillCards.filterizr('filter', filter);
      });
    }
    ...
    if (projectCardHolder != null && projectCardHolder.children.length != 0) {
      ...
      $('#project-filter-buttons button').click(function () {
        var filter = $(this).data('filter');
        projectCards.filterizr('filter', filter);
      });
    }
